...
// Valores dinâmicos resolvidos no init
let API_URL    = '';
let API_KEY    = '';
let API_SECRET = '';
let WHATSAPP   = '';
  ...
  function renderFilter() {
    ...
    filterEl.innerHTML = cats.length ? `
      <div class="svc-cat-tabs">
        ...
        ${cats.map(c => `<button class="svc-cat-tab${activeCat === c.id ? ' active' : ''}" data-cat="${c.id}">${c.name}</button>`).join('')}
      </div>` : '';
    filterEl.querySelectorAll('.svc-cat-tab').forEach(btn => {
      btn.addEventListener('click', () => { activeCat = btn.dataset.cat; renderFilter(); renderCards(); });
    });
  }
...
function renderSiteTeam() {
  ...
  container.querySelectorAll('.btn-equipe-agendar').forEach(btn => {
    btn.addEventListener('click', () => document.getElementById('servicos')?.scrollIntoView({ behavior: 'smooth' }));
  });
}
...
function updateModalFooter() {
  ...
  const { step } = state;
  if (step === 7) { btnConfirmar.style.display = 'inline-flex'; return; }
...
function renderStep3() {
  ...
  container.querySelectorAll('.emp-item').forEach(item => {
    item.addEventListener('click', () => {
      ...
      state.tempHora = null; state.allSlots = []; state.availSlots = [];
      renderStep3();
...
function renderStep7() {
  ...
  container.innerHTML = `
    ...
    <div class="confirm-item-box">
      <div class="confirm-item-header"><span>Agendamento · ${item.funcionarioNome}</span></div>
...
function initConfigModal() {
  ...
  // Banner de aviso se não configurado
  const cfg = loadConfig();
...
// ─── APPLY CONFIG — aplica valores dinâmicos ao DOM ─────────────────────────
...
function applyConfig() {
  ...
  // Atualiza nome da empresa no DOM
...
async function init() {
  ...
  // Botões "Agendar" genéricos vão para a seção de serviços
  ...
  if (successClose) {
    successClose.addEventListener('click', () => { resetFullBooking(); closeBookingModal(); });
  }
  ...
  if (API_URL && API_KEY) {
    ...
    // Sem config — mostrar estado vazio amigável
    document.getElementById('site-services-grid').innerHTML =
      `<p style="text-align:center;color:#555;padding:40px">
         Configure a integração para exibir os serviços.
         <br><br>
    ...
    document.getElementById('site-team-grid').innerHTML =
      '<p style="text-align:center;color:#555;padding:40px">Configure a integração para exibir a equipe.</p>';
    document.getElementById('site-products-grid').innerHTML =
      '<p style="text-align:center;color:#555;padding:40px">Configure a integração para exibir os produtos.</p>';
  }
}

// Expor openConfigModal globalmente para o botão inline no estado vazio
